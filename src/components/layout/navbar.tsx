  ...
  return (
    <nav className="sticky top-0 z-50 bg-[#F7F7F2]/90 backdrop-blur-md border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative flex md:flex md:items-center md:justify-between items-center h-20">
          {/* Logo: center absolute di mobile, statis di desktop (kiri) */}
          <div className="absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0 shrink-0 flex items-center whitespace-nowrap">
          ...
          <div className="hidden md:flex flex-1 justify-center space-x-8 relative z-10">
            {links.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-gray-600 hover:text-[#1e3b2b] whitespace-nowrap"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center justify-end gap-4 shrink-0 relative z-10">
      ...
      {isOpen && (
        <div className="md:hidden bg-[#F7F7F2] border-t border-gray-200 px-4 py-4 flex flex-col space-y-4">
          ...
          <div className="border-t border-gray-200 pt-4 flex items-center gap-5">
            {socials.map(({ href, icon: Icon, label }) => (
              <Link
                key={label}
                href={href}
